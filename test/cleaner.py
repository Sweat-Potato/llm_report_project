...
# 자료 출처
# 예: "자료: www.nature.com, 유안타증권 리서치센터"
...
# 표/리스트 앞에 오는 제목 패턴
# 예: "Performance / Valuation", "국내외 반도체 서플라이체인"
_TABLE_TITLE_RE = re.compile(
    r"(Performance|Valuation|서플라이체인|Supply.?Chain"
    r"|전방산업|후방산업|관련\s*기업|수혜\s*기업|Coverage"
    r"|업체\s*현황|기업\s*현황|종목\s*리스트)",
    re.IGNORECASE,
)
...
def _is_company_name(s: str) -> bool:
    ...
    return (
        ...
        and not re.search(r"[가-힣]{6,}", s)  # 긴 한글 문장 제외
        and s not in ("흑전", "적전", "적지", "흑지", "흑자전환", "적자전환")  # 단독 재무용어 - 블록 내에서는 허용
    )

# 표 안에서 자주 등장하는 재무 상태 단어
_FINANCIAL_STATUS = {"흑전", "적전", "적지", "흑지", "흑자전환", "적자전환",
                     "N/A", "n/a", "-", "--", "N.A.", "적자", "흑자"}

def _is_entity_name_block(segment: list[str]) -> bool:
    """
    연속 줄이 회사명/고유명사 나열인지 판단
    - 3줄 이상 연속
    - 80% 이상이 회사명 or 재무상태 단어
    """
    if len(segment) < 3:
        return False
    valid_count = sum(
        1 for s in segment
        if _is_company_name(s) or s in _FINANCIAL_STATUS
    )
    return valid_count >= len(segment) * 0.8
...
def _remove_tables_charts(text: str) -> str:
    ...
    while i < len(lines):
        ...
        # 고유명사 나열 블록 감지 (업체명 목록)
            ...
            while j < len(lines):
                ...
                if not s:
                    ...
                    if blank_count <= 2:
                        ...
                        break
                blank_count = 0  # 내용 있는 줄 나오면 빈 줄 카운트 리셋
                if _is_table_chart_line(lines[j]):
                    j += 1
                    continue
                if _is_company_name(s) or s in _FINANCIAL_STATUS:
                    j += 1
            ...
            if _is_entity_name_block(segment):
                # 업체명 나열 블록 → 제거
                i = j
                continue
...
def _remove_page_numbers(text: str) -> str:
    ...
    # pymupdf4llm이 생성하는 마크다운 형식 페이지번호: "###### 04", "####### 12" 등
...
def clean_page(text: str) -> str:
    """단일 페이지 텍스트 정제 (loader의 raw 텍스트 입력)"""
    # 0. HTML 태그 제거
