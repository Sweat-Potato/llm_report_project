...
개별 이벤트 해석이나 산업 함의 분석은 포함되지 않는다.
...
- PE·PP·MEG 등 만성 공급과잉 제품도 가격 강세 가능성
- "전쟁 후 강한 재고 비축이 항상 이어졌다"는 역사적 패턴 인용
...
구조적으로는 수급 불균형 심화와 AI 수요 의존도가 주요 변수로 제시된다.  
...
**④ AI 수요 의존도 (대신증권, 2026-04-06)**  
- 메모리 업황이 AI 투자 흐름에 크게 의존  
- 수요 사이클 변화 시 업황 변동 가능성 존재
...
- 발췌문에 없는 내용은 추정하지 말고 "리포트에서 확인되지 않음"이라고 쓰세요.
...
다른 증권사와 비교할 수 없는 경우에도,
이 리포트 내부에서 드러나는 독자적 분석 프레임을 설명하세요.
""")
...
향후 어떤 데이터가 나오면 이견이 해소되거나 심화될지 정리하세요.
...
현재 자료상 중요하지만 충분히 강조되지 않은 변수를 정리하세요.

각 변수별로:
- 왜 과소평가될 수 있는가
- 어떤 조건에서 부각되는가
- 부각될 경우 업황·실적·밸류에이션 해석이 어떻게 바뀌는가

---

## 3. 단기 vs 중장기 전망의 괴리

3~6개월 단기 전망과 1~2년 중장기 전망이 다르게 나타나는 부분을 분석하세요.

포함:
- 단기에는 긍정적이나 중장기에는 리스크인 요소
- 단기에는 부담이나 중장기에는 기회인 요소
- 증권사별 시간축 차이
- 어떤 데이터가 전망 전환의 트리거가 되는지

---

## 4. 시나리오 구조

Bull / Base / Bear 시나리오를 작성하세요.
...
- 발췌문에 존재하는 **구체적 수치만 인용**
- 모든 수치에는 반드시 **출처(증권사, 날짜)** 표기
- 발췌문에 없는 정보는 절대 생성 금지 → "리포트에서 확인되지 않음" 명시
- 일반론 / 산업 상식 / 교과서 설명 금지
...
- Bull Case → 어떤 조건에서 성립하는가  
- Base Case → 현재 시장이 반영한 시나리오  
- Bear Case → 어떤 변수에서 깨지는가  
